...
def colorit(image):
    ...
    args = {
        "prototxt": "colorizer/model/ptxt.prototxt",
        "model": "colorizer/model/cm.caffemodel",
        "points": "colorizer/model/ptc.npy"
    }
    ...
    scaled = image.copy()
    scaled = scaled.astype("float32") / 255.0
    ...
    retval, before_buffer = cv2.imencode('.jpg', image)
    retval, after_buffer = cv2.imencode('.jpg', colorized)

    return (base64.b64encode(before_buffer).decode(), base64.b64encode(after_buffer).decode())
